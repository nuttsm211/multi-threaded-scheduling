/* ============================================================================
   PROJECT: Multithreaded RTOS Simulation on LPC17xx
   DESCRIPTION:
   This program demonstrates thread creation, synchronization, and termination 
   using CMSIS-RTOS on an LPC17xx microcontroller. It simulates different system 
   components (Memory, CPU, Application, Device, and User Interface) using 
   independent threads. Each thread performs simple operations, displays its 
   activity on the GLCD, and controls LEDs to visualize thread execution order.

   Threads:
   - MemoryThread: Handles simulated memory operations.
   - CPUThread: Performs arithmetic operations (power calculations).
   - AppThread: Logs data and updates GLCD.
   - DeviceThread: Updates shared variable and signals AppThread.
   - UIThread: Simulates user interface actions.

   Visual Output:
   - LEDs represent active threads.
   - GLCD displays the currently executing thread and logger values.
============================================================================ */
...
/* System Initialization and Thread Start */
int main(void) {
  LED_Init();
#ifdef __USE_LCD
  GLCD_Init();
